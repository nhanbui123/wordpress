...
if ( ! class_exists( 'YITH_WC_Share_For_Discounts' ) ) {
	...
	class YITH_WC_Share_For_Discounts {
		...
		public function frontend_scripts() {
			...
			if ( ! apply_filters( 'ywsfd_can_get_coupon', true, ( isset( $post->ID ) ? $post->ID : '' ) ) || $this->coupon_already_assigned() || ( is_shop() && apply_filters( 'ywsfd_show_on_shop', false ) == false ) || is_account_page() ) {
				return;
			}
		...
		/**
		 * Warn users about Google+ Deprecation
		 *
		 * @since   1.0.0
		 * @return  void
		 * @author  Alberto Ruggiero
		...
		public function set_notices() {
				...
				if ( get_option( 'ywsfd_enable_google' ) == 'yes' && empty( $_COOKIE['ywsfd_warning'] ) ) {

					?>
                    <div id="ywsfd-warning" class="notice notice-warning" style="position: relative;">
                        <p>
							<?php _e( 'Warning: due to the removal by Google+ of the features that enabled the plugin to know if the sharing was successful, we removed the support for Google+ from the plugin.', 'yith-woocommerce-share-for-discounts' ); ?>
                        </p>
                        <span class="notice-dismiss ywsfd-warning"></span>
                    </div>
					<?php
		...
		public function dismissable_notice() {
			...
			$js = "jQuery(document).on( 'click', '.ywsfd-warning', function(){jQuery( '#ywsfd-warning' ).animate({ opacity: 0.25, height: 'toggle' }, 650 );Cookies.set( 'ywsfd_warning', 'dismiss', {path: '/'} );}); ";
