...
class Stylist_Class_Manager {
	...
	public function add_class( $wp_block_content, $new_class ) {
		...
		if ( false !== $class_attr_pos ) {
			...
			// to his taste without any particular login.
			...
			// Replace class attribute value with our custom class added.
			// https://regex101.com/r/w4uWgb/3
			$re = '/(class="[a-zA-Z\-_\s]*' . $class_attr_val . '[a-zA-Z\-_\s]*)/';
			$subst = '$1 ' . $new_class;
	...
	public function create_class_attr_dom( $wp_block_content, $new_class ) {
		...
		// Prevent detectng "-->" cases as valid openning tag end.
		while ( isset( $wp_block_content[ intval($first_tag_pos) - 1 ] ) && '-' === $wp_block_content[ intval($first_tag_pos) - 1 ] ) {
			$first_tag_pos = strpos( $wp_block_content, '>', intval($first_tag_pos) + 1 );
		}

		// Split string after first tag end.
		if ( false !== $first_tag_pos ) {
			$first_tag_pos = intval($first_tag_pos)+1;

			// String parts:
			$first_tag = substr ( $wp_block_content , 0, $first_tag_pos );

			$class_pos = strpos( $first_tag, 'class="' );

			// No class="" atribute found.
			if ( false === $class_pos ) {
				$first_tag_with_class = str_replace ('>', ' class="">', $first_tag );
				$wp_block_content = str_replace( $first_tag, $first_tag_with_class, $wp_block_content );
			...
			$wp_block_content = substr_replace( $wp_block_content,  $new_class . ' ', $class_pos, 0);
